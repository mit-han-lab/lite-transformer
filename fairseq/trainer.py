...
class Trainer(object):
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        #     print('| WARNING: scale too small, skip batch')
        #     self.zero_grad()
        #     logging_output = None
    ...
    def test_bleu(self, sample, bleu_scorer):
        if sample is None:
            return

        beam = self.args.beam
        with torch.no_grad():
            hypos = self.task.inference_step(self._generator, [self.model], sample, None)
            ...
            for i, sample_id in enumerate(sample['id'].tolist()):
                ...
                target_str = tgt_dict.string(target_tokens, self.args.remove_bpe, escape_unk=True)
                
                hypo = hypos[i][0]
                hypo_tokens = hypo['tokens'].int().cpu()
                hypo_str = tgt_dict.string(hypo_tokens, self.args.remove_bpe)

                if hasattr(bleu_scorer, 'add_string'):
                    bleu_scorer.add_string(target_str, hypo_str)
                else:
                    bleu_scorer.add(target_tokens, hypo_tokens)

    def test_bleu_step(self, sample, bleu_scorer, print_to_file=None):
        with torch.no_grad():
            self.model.eval()
            sample = self._prepare_sample(sample)
            self.test_bleu(sample, bleu_scorer)
